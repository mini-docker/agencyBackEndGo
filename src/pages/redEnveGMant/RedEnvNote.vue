		...
		<div class="container">
			<div class="search-box">
				...
				<div class="pk-inp-box">
					...
					<el-input v-model="query.inpstr" :placeholder="$t('common.p_KeywordS')" class="selbox"></el-input>
				</div>
                <div class="pk-inp-box pk-select">
			...
			<div class="handle-box">
				<el-tabs v-model="activeName" type="card" @tab-click="query.status = 10;getData()" class="pull-width">
                    ...
                    <!-- 牛牛 经典 平倍 超倍 -->
						<div v-show="item.key === 1||item.key === 3||item.key === 4">
							<el-table :data="tableData" @selection-change="handleSelectionChange" :height="fData.tableHeight()+10"  border class="table" ref="multipleTable" header-cell-class-name="table-header">
								...
								<!-- <el-table-column prop="memberNum" :label="$t('common.odds')" align="center">
                                    <template slot-scope="scope">
                                        <el-tag
                                            :type="scope.row.odds>0?'success':scope.row.odds<0?'danger':''"
                                        >{{scope.row.memberNum|filterfuaddOdds(scope.row.gamePlay)}}</el-tag>
                                    </template>
                                </el-table-column> -->
                                ...
                                <!-- <el-table-column prop="isRobot" :label="$t('common.robotOrder')" align="center">
                                    <template slot-scope="scope">
                                        ...
                                        >{{scope.row.isRobot|filterIsTrue}}</el-tag>
                                    </template>
                                </el-table-column> -->
            ...
            <el-dialog :title="$t('common.detRedGet')" :visible.sync="dialogTableVisible">
                <el-table border :data="rows">
                    <el-table-column property="account" :label="$t('common.userName')" align="center" width="200">
                        <template slot-scope="scope">
                            <div>
                                {{scope.row.account}}
...
export default {
	...
	methods: {
        ...
        handleEdit(row){
            getAGEorderDet({redId:row.redId}).then(res => {
                if(res.success){
